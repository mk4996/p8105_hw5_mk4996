...
I created `city_state` (e.g., “Baltimore, MD”) and summarized within cities to obtain the total number of homicides (*`n_total`*) and the number of unsolved homicides (`n_unsolved`), where unsolved is defined as *"Closed without arrest"* or *"Open/No arrest"*.
Chicago, IL had the highest total number of homicides (`r format(chi_total, big.mark=",")`) and one of the highest proportions of unsolved cases (`r chi_prop`%), followed by Baltimore, MD (`r bal_prop`%) and New Orleans, LA (`r nola_prop`%). In contrast, Richmond, VA (`r rich_prop`%) and Charlotte, NC (`r charl_prop`%) had the lowest proportions of unsolved cases, highlighting substantial variation across cities.
